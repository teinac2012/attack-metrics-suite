<!-- Utilidad de persistencia de datos entre apps -->
<script>
  ...
  window.AnalysisDataSync = {
    ...
    saveProgress: function(appId, data) {
      ...
      sessionStorage.setItem('sync_update', JSON.stringify({ appId, timestamp: Date.now() }));
    },
    ...
    saveMatchContext: function(matchId, context) {
      localStorage.setItem('last_match_context', JSON.stringify({
        matchId: matchId,
        context: context,
        timestamp: new Date().toISOString()
      }));
